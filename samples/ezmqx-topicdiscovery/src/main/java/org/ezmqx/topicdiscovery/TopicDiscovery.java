...
public class TopicDiscovery {
	...
	private static void printError() {
		...
		System.out.println("      $ java -jar ezmqx-topicdiscovery-sample.jar -t /a/b/c -tns 192.168.8.6");
	...
	public static void main(String[] args) {
		...
		try {
			...
			if (isStandAlone) {
				config.startStandAloneMode("", true, tnsAddress);
			} else {
		...
		if (!isStandAlone) {
			try {
				System.out
						.println("Waiting for 5 minutes before program exit for docker mode... [press ctrl+c to exit]");
				Thread.sleep(300000);
