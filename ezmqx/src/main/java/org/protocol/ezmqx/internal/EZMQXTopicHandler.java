...
public class EZMQXTopicHandler {
    private static EZMQXTopicHandler mInstance;
    private EZMQXRestClient mRestClient;
    private ZMQ.Context mContext;
    private EZMQXContext mEZMQXContext;
    ...
    private final static EdgeXLogger logger =
            EdgeXLoggerFactory.getEdgeXLogger(EZMQXTopicHandler.class);

    private EZMQXTopicHandler() throws EZMQXException {
        mContext = EZMQAPI.getInstance().getContext();
        mEZMQXContext = EZMQXContext.getInstance();
    ...
    public static synchronized EZMQXTopicHandler getInstance() throws EZMQXException {
        if (null == mInstance) {
            mInstance = new EZMQXTopicHandler();
        }
        return mInstance;
    }
    ...
    public void updateKeepAliveInterval(int keepAliveInterval) {
        mRestClient = new EZMQXRestClient(mKeepAliveInterval.get() * 2);
    ...
    private void sendKeepAlive() {
        String keepAliveURL = EZMQXRestUtils.HTTP_PREFIX + mTnsAddress + EZMQXRestUtils.COLON
                + EZMQXRestUtils.TNS_KNOWN_PORT + EZMQXRestUtils.PREFIX
                + EZMQXRestUtils.TNS_KEEP_ALIVE;
        ...
        try {
            Response response = mRestClient.post(keepAliveURL, payload);
            logger.debug("[TNS Keep alive topic] Response code: " + response.getStatus());
        } catch (Exception e) {
    ...
    private boolean parseSocketData() {
        ...
        if (requestType.equals(EZMQXRestUtils.SHUTDOWN)) {
            logger.debug("requestType: " + requestType);
            return true;
        } else if (requestType.equals(EZMQXRestUtils.REGISTER)) {
            AddTopic(data);
        } else if (requestType.equals(EZMQXRestUtils.UNREGISTER)) {
            removeTopic(data);
        } else if (requestType.equals(EZMQXRestUtils.KEEPALIVE)) {
